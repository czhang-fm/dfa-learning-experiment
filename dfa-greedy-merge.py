...
# where d is the maximal length of a training string, and -1, 0, 1 represent "don't know", "non-accept", and "accept", respectively.
import random
...
def incomp_class(state_one, state_two, merged_classes, incomp, states, cache, id_state, transition):
    """check if state_one and state_two are incompatible at the level of equivalence classes"""
    desc = False
    state_one = id_state[state_one]
    state_two = id_state[state_two]
    if (state_one, state_two) in cache:
        return False
    
    cache.add((state_one, state_two)) # state_one and state_two are compatible
    cache.add((state_two, state_one))
    
    if state_one in incomp[state_two]:
        return True
    if (state_one, '0') in transition and (state_two, '0') in transition:
        desc = incomp_class(transition[(state_one,'0')], transition[(state_two,'0')], merged_classes, incomp, states, cache, id_state, transition)
        if desc:
            return True
    if (state_one, '1') in transition and (state_two, '1') in transition:
        desc = incomp_class(transition[(state_one,'1')], transition[(state_two,'1')], merged_classes, incomp, states, cache, id_state, transition)
        if desc:
            return True
    
    return False

def merge(state_one, state_two, eq_classes, merged_classes, incomp, comp, states, id_state, transition):
    ...
    # before merging the two states, we update the compatibility and incompatibility relations:
    # For all s with comp(s, state_one) and s\not\in new_set, if there exists t in set_two with incomp(s, t),
    # we add (s, t') to incomp and remove (s,t') from comp, for all t'\in new_set, symmetrically
    # Similarly, for all t with comp(t, state_two) and t\not\in new_set, if there exists s in set_one with incomp(s, t)
    # we add (s', t) to incomp and remove (s', t) from comp, for all s'\in new_set, symmetrically
    # INVARIANT: for all s\not\in new_set, then either comp(s, t) for all states t in new_set, or incomp(s, t) for all states t in new_set
    for s in states:
        if s in new_set: #or (s in incomp[state_one]):
            continue
        for t in new_set:
            if t in incomp[s]:
                for t2 in new_set:
                    if t2 in comp[s]:
                        ...
                        incomp[t2].add(s)
                break
    # Next we merge set_one and set_two
    for s in new_set:
        merged_classes[s] = new_set
    seed = id_state[state_one] # get the id of the first eq class
    seed2 = id_state[state_two]
    for s in set_two:
        id_state[s] = seed
    if state_two in eq_classes:
        eq_classes.remove(state_two)

    # if transition[(seed,a)] is not defined, borrow the transition[(seed2, a)] if it's defined for seed2
    if (seed, '0') not in transition:
        if (seed2, '0') in transition:
            transition[(seed, '0')] = transition[(seed2, '0')]
    if (seed, '1') not in transition:
        if (seed2, '1') in transition:
            transition[(seed, '1')] = transition[(seed2, '1')]
    
    if state_one+'0' in states and state_two+'0' in states:
        merge(state_one+'0', state_two+'0', eq_classes, merged_classes, incomp, comp, states, id_state, transition)
    
    if state_one+'1' in states and state_two+'1' in states:
        merge(state_one+'1', state_two+'1', eq_classes, merged_classes, incomp, comp, states, id_state, transition)
    

def det_merge_states(eq_classes, incomp, comp, states): 
    ...
    id_state = {} # the representative state in each equivalence class
    transition = {} # save this to use later ......
    for state in eq_classes:
        merged_classes[state] = {state} # initialize merged_classes as the identity relation
        id_state[state] = state
        if state+'0' in eq_classes:
            transition[(state, '0')] = state+'0'
        if state+'1' in eq_classes:
    ...
    while eq_classes != []:
        ...
        idx = 1
        while incomp_class(state_one, eq_classes[idx], merged_classes, incomp, states, set(), id_state, transition): 
        ...
        merge(state_one, eq_classes[idx], eq_classes, merged_classes, incomp, comp, states, id_state, transition)
...
def generate_dfa(state_set, merged_classes, data):
    ...
    while dfa != []: # dfa stores what have not yet computed
        ...
        if right != 'None': 
            ...
            print("Error: right not found!")
    return visited, label, transition_left, transition_right

def main():
    """the starting point"""
    data = produce_raw_data("~/train0large.txt")
    ...
    eq_classes.sort(reverse=True, key=lambda x: weights[x])
    #print(eq_classes)
    #return
    ...
    merged_classes = det_merge_states(eq_classes, incomp, comp, states)
    ...
    results, test_data = read_test_data("~/train0large.txt")
    ...
    for actions in test_data:
        ...
        if int(results[r_idx]) != label[my_state]:
            print(f"Error in the {r_idx}th test case")
            error_num += 1
        r_idx += 1
    print(f"Produced {error_num} errors out of {len(results)} test cases!")
